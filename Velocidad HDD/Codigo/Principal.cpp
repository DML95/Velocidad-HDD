...
const char *nombreClase="VelocidadHDD";

Ventana *pVentana;
Tabla *pTabla;
Dispositivos *pDispositivos;
Temporizador *pTemporizador;

void error(char *error,int num){
	char texto[0xff];
...
int bucleMensajes(HWND hVentana){
	MSG mensaje;
...
WNDPROC evento(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam){
	int salida=0;
	switch(msg){
...
int main(int argc, char** argv) {
	...
	if(hVentana){
		...
		try{
			...
			try{
				Clase clase=Clase((char*)nombreClase,(WNDPROC)&evento);
				try{
